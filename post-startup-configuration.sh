...
if [ "$DS_PACKAGES" = true ]; then
  # Python
  if [ "$INSTALL_PY3_PKGS" = true ]; then
    sudo python3 -m pip install scikit-learn pandas numpy numexpr statsmodels scipy xgboost h2o
  else
    sudo python -m pip install scikit-learn pandas numpy numexpr statsmodels scipy xgboost h2o
  fi
  # Javascript
  if [ "$JS_KERNEL" = true ]; then
